# Purpose:
    ...
    # It supports:
        # Receiling task requests via POST ("/")
        #-Lettis clients discover the agent's details via GET ("/.well-known/agent.json")
...
# Importing the custom models & logic
...
# used to make sure non-JSON-serializable types (like datetime) can be correctly converted into JSON format.
...
# -----------------------------------------------------------
class A2AServer:
    def _init_(self, host="0.0.0.0", port=5000, agent_card: AgentCard = None, task_manager: task_manager = None):
        """
            Constructor for the A2AServer

            Args:
                host: IP address to bind the server to (default is all interfaces)
                port: Port number to listen on (default is 5000)
                agent_card: Metadata that describes the agent (name, skills, capabilities)
    ...
    def start(self):
        """
            Starts the A2A sever using uvicorn (ASGI web server).
            This function with block and run the server forever.
        """
    ...
    # --------------------------------------------------------------
    # _create_response(): Converts result obj to JSONResponse
    # --------------------------------------------------------------
    def _create_response(self, result):
        """
            ...
            Returns:
                JSONResponse: Starlette-compatible HTTP response with
        """
        ...
        if isinstance(result, JSONRPCResponse):
            ...
            logger.error("Invalid response type returned from task manager")
            return JSONResponse(
                content=JSONRPCResponse(
                    id=None,
                    error=InternalError(message="Invalid response from task manager")
                ).model_dump(),
                status_code=500
            )