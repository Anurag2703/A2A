# Purpose:
    ...
    # It:
        ...
        # Sets upe A2A server with a task manager and agent
...
# Custom A2A server class
from server.server import A2AServer

# Models for describing agent capabilities & metadata
...
# -------------------------------------------------
# 2. Setup logging to print info to the console
# -------------------------------------------------
...
# -------------------------------------------------
# 3. Main Entry function: Configurable via CLI
# -------------------------------------------------
...
def main(host, port):
    ...
    # Crate an agent card describing this agent's identity and metadata
    ...
    # Start the A2A server with with:
        # 1. the given host/port
        # 2. this agent's metadata
        # 3. a task manager that runs the TellTimeAgent
    server = A2AServer(
        host = host,
        port = port,
...
# --------------------------------------------------------------------------
# 4. This will only run when executing the script directly via 'python -m'
# --------------------------------------------------------------------------
